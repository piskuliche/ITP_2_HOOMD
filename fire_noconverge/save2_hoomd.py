...
def Generate_Snapshot(snapshot,mol_ff,forcefield,sysinfo):
...
if __name__ == "__main__":
    ...
    # Set bonds,angles,dihedrals between atoms
    snapshot.pairs = set_snap(snapshot.pairs,ff["DOPC"], ff["DOPC"].bonds,sysinfo["DOPC"])
    ...
    for pair in snapshot.pairs.types:
        ...
        spec_lj_forces.r_cut[pair] = 1.2
        spec_coul_forces.params[pair] = dict(alpha=1)
        spec_coul_forces.r_cut[pair] = 1.2
    ...
    integrator = hoomd.md.Integrator(dt=0.001,
                                     methods=[ensemble],
                                     forces=[spec_lj_forces,spec_coul_forces,ew,coul,bondforces,angleforces,dihedralforces])
